...
<body>
	  	...
	  	<div class="page-content clearfix">
		    <div id="Member_Ratings">
		     ...
		     <div class="border clearfix">
		       ...
		       <span class="r_f">共<b>${fn:length(listHotelOrder) }</b>个订单记录</span>
		     </div>
		     <!---->
		       ...
		       <table class="table table-striped table-bordered table-hover" id="sample-table">
				<thead>
				 <tr>
						...
						<th width="160">订单需求</th>
						<th width="110">操作</th>
					</tr>
				</thead>
			<tbody>
				
		       <c:forEach items="${listHotelOrder }" var="s" varStatus="sg">
			          ...
			          <td>${s.thrGoaddress }</td>
			          <td>${s.thrDates }</td>
			          <td>${s.thrNum }</td>
			          <td>${s.thrOther }</td>
			           <!-- <td class="td-status"><span class="label label-success radius">已启用</span></td> -->
			          <td class="td-manage">
			         	 <a title="编辑订单状态" onclick="member_edit()" href="javascript:;"  class="btn btn-xs btn-info" ><i class="icon-edit bigger-120"></i></a> 
			          </td>
					</tr>
		       </c:forEach>
  ...
  <script>
	...
	function member_edit(orderId,statusId){
		...
		if(statusId==0){
		$.post(
				"<%=basePath%>lasterOrder/updateSceneByOrderMsg",
				{"orderId":orderId},
				function(msg){
					if(msg>0){
						layer.msg('订单信息已查看!',{icon:1,time:1000});
						location.href = "<%=basePath%>lasterOrder/selectOrderBySceneList" ;
					}else{
