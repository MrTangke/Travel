...
<title>个人中心</title>
	...
	<%-- <link rel="stylesheet" type="text/css" href="<%=basePath %>styles/before/css/mycenter.css" />
	<script type="text/javascript" src="<%=basePathJs %>styles/before/Js/mycenter.js"></script> --%>
		...
		<div class="public_box">
			...
			<div id="set_box">
				...
				<table class="table_list">
					<tr>
						<th>旧密码：</th>
						<td><input class="text" type="password" id="outPassword" onchange="comparing(${sessionScope.tourist.touristId })" value=""/></td>
					</tr>
					<tr>
						<th>新密码：</th>
						<td><input class="text" type="password" id="pwd1" value=""/></td>
					</tr>
					<tr>
						<th>再次输入：</th>
						<td><input class="text" type="password" id="pwd2" onchange="comparingTwo()" value=""/></td>
					</tr>
				</table>
				<div class="operation"><input type="submit" id="sub" onclick="savePassword(${sessionScope.tourist.touristId })" class="save" value="保存" /></div>
...
<script type="text/javascript">
	function comparing(touristId){
		var pwd = $("#outPassword").val();
		$.post(
			"<%=basePath%>tourist/comparingPwd",
			{"pwd":pwd,"touristId":touristId},
			function(msg){
				if(msg==0){
					layer.msg('密码不匹配，请核对密码！',{icon:1,time:1000});
	...
	function comparingTwo(){
		...
		var pwd2 = $("#pwd2").val();
		if(pwd1 != pwd2){
			layer.msg('两次输入密码不一致!',{icon:1,time:1000});
			$("#sub").attr('disabled',true);
		}else{
			$("#sub").attr('disabled',false);
		}
	...
	function savePassword(touristId){
		var pwd = $("#outPassword").val();
		var pwd1 = $("#pwd1").val();
		var pwd2 = $("#pwd2").val();
		if(pwd != "" && pwd != null){
			if(pwd1 != "" && pwd1 != null){
				if(pwd2 != "" && pwd2 != null ){
					$.post(
						"<%=basePath%>tourist/updateTouristIdPassword",
						{"touristPassword":pwd1,"touristId":touristId},
							function(msg){
								if(msg==1){
					...
					layer.msg('再次输入密码不能为空！',{icon:1,time:1000});
				}				
			}else{
				layer.msg('新密码不能为空！',{icon:1,time:1000});
			}
		}else{
			layer.msg('旧密码不能为空！',{icon:1,time:1000});
		}
	}
</script>
</html>